(() => {
    const FIXES = [
        ...
        {"id": "FIX-124", "title": "Wallet deadlock investigation and evidence-based repair", "status": "PLANNED — TO BE DONE", "scenario": "Team reports wallet_managed_position deadlocks; incidents and lock ordering require independent verification.", "cause": "Actual deadlock cause is not yet established here. A possible transaction visibility race is also unproven and must not be conflated with deadlocks.", "solution": "Inspect incident/deadlock evidence, reproduce the failure, and propose one scoped correction with concurrency and rollback tests. Transaction monitor-release/commit coordination is reserved separately as FIX-125.", "behavior": "Planning only. No wallet locking, transaction ordering or trading behavior changed by this entry.", "regression": "Pending investigation and separately reviewed implementation.", "symbol": "ALL", "entry": "PLANNED", "exit": "UNCHANGED", "entryTime": "NOT IMPLEMENTED", "exitTime": "UNCHANGED", "replayWindow": "Identical OLD/NEW windows with complete preceding stop and winning/losing controls.", "location": "Separate planned work", "classes": ["md/FIX-124.md"]},
