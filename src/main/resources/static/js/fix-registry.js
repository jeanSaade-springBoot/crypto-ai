(() => {
    const FIXES = [
        {id:"FIX-118",title:"Profit Lock TP-extension trace instrumentation",status:"IMPLEMENTED · PRODUCTION/REPLAY OBSERVABILITY ONLY · NO TRADING SEMANTIC CHANGE",scenario:"Trace the exact Dynamic Profit Lock state/write sequence when an already-protected winning position reaches TP, continuation extends the target, and the next live-price or candle-close evaluation can still trigger the previously-earned lock under the wider geometry.",symbol:"ALL LONG POSITIONS (primary evidence ICPUSDT #1010 / #1014)",entry:"UNCHANGED",exit:"UNCHANGED · TRACE ONLY",entryTime:"Existing position lifecycle",exitTime:"Existing Production/Replay exit authority",replayWindow:"Primary validation: ICPUSDT #1010 around 2026-09-06 12:58:03→13:04:30 UTC (15:58:03→16:04:30 KSA) and #1014 around 13:41:03→14:08:03 UTC (16:41:03→17:08:03 KSA).",location:"DynamicProfitLockService + LivePositionProtectionService + PaperTradingService + ShadowProductionReplayService server/replay trace logs",classes:["DynamicProfitLockService","LivePositionProtectionService","PaperTradingService","ShadowProductionReplayService","fix-registry.js","md/FIX-118.md"],cause:"Investigation confirmed two independent Production evaluators (live-price protection and PaperTradingService signal processing) plus Replay equivalents. TP-extension ticks return before Profit Lock evaluation, while previouslyActive/previousLock persist into the wider target geometry. Before changing behavior, the exact caller, geometry, progress, lock and trigger sequence must be visible directly in server and Replay logs.",solution:"Add FIX-118 diagnostic logs at TP extension, shared ProfitLockPolicy persistence/evaluation, live-price trigger authority, candle/signal trigger authority, and both Replay live-price/signal paths. Logs include position/run/signal identity, entry, old/new/current TP, highest, previous/current active state, previous/current lock, progress, activation and triggered state. No REBASE state or exit behavior is introduced in this instrumentation phase.",behavior:"Observability only. BUY/SELL logic, TP extension, Dynamic Profit Lock activation/initial/trail math, hard floor, Stop Loss, FIX-117 Near-TP behavior, wallet execution and Replay baseline outcomes remain unchanged. The instrumentation exists to prove the exact path before the separately-reviewed FIX-118 behavioral correction is authorized.",regression:"Compile/test existing suite; replay ICP #1010/#1014 and verify [FIX-118][REPLAY] traces identify TP extension then the subsequent Profit Lock state/trigger. On Production, verify [FIX-118][PRODUCTION] distinguishes LIVE_PATH/LIVE_TRIGGER from SIGNAL_PATH/SIGNAL_TRIGGER and shared PROFIT_LOCK_EVAL persistence without changing any exit result."},
