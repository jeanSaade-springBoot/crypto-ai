(() => {
    const FIXES = [
        ...
        {id:"FIX-118",title:"Profit Lock TP-extension rebase",status:"IMPLEMENTED · PRODUCTION/REPLAY TRADING-SEMANTIC CORRECTION",scenario:"An already-earned Dynamic Profit Lock can remain executable after continuation extends TP even though progress against the wider current target falls back below the configured activation threshold. ICPUSDT #1010/#1014 proved the historical lock could terminate an otherwise approved continuation.",symbol:"ALL LONG POSITIONS (primary evidence ICPUSDT #1010 / #1014)",entry:"UNCHANGED",exit:"FIX-118 TP_EXTENSION_REBASE STATE",entryTime:"Existing position lifecycle",exitTime:"Live-price and authoritative signal-price protection",replayWindow:"Primary validation: ICPUSDT #1010 around 2026-09-06 12:58:03→13:04:30 UTC (15:58:03→16:04:30 KSA) and #1014 around 13:41:03→14:08:03 UTC (16:41:03→17:08:03 KSA).",location:"DynamicProfitLockService + LivePositionProtectionService + PaperTradingService + ShadowProductionReplayService + persisted wallet/replay state",classes:["ProfitLockState","WalletManagedPosition","DynamicProfitLockService","LivePositionProtectionService","PaperTradingService","ShadowProductionReplayService","V82__fix_118_profit_lock_tp_extension_rebase.sql","DynamicProfitLockServiceTest","LivePositionProtectionServiceTpExtensionSyncTest","fix-registry.js","md/FIX-118.md"],cause:"Profit Lock legitimately activates under the pre-extension TP geometry. After an approved TP extension, the legacy active flag/lock remain sticky even when progress recalculated against the wider target is below activation, so a lock earned against superseded geometry can still execute.",solution:"Persist explicit INACTIVE/ACTIVE/TP_EXTENSION_REBASE state. If an ACTIVE lock falls below activation under the newly extended TP, keep its historical flag/price persisted but make the old lock non-executable. Continue tracking highest/progress against the current TP; reactivate at the configured activation threshold. During rebase keep Stop Loss absolute and enforce a standalone entry×1.0005 protected-profit floor. Apply identical state semantics to Production live/signal paths and Replay live/signal paths.",behavior:"Intentional narrow exit-management correction. TP-extension eligibility/formula and Profit Lock activation/initial/trail percentages are unchanged. Normal thesis exits remain available during rebase. FIX-117 Near-TP and FIX-119 entry timing remain unchanged.",regression:"Verify ACTIVE→TP_EXTENSION_REBASE when new-geometry progress falls below activation; old lock cannot trigger during rebase; rebase releases to ACTIVE at current-TP activation; standalone protected-profit floor triggers PROFIT_LOCK_HARD_EXIT; migration backfills legacy active rows; Production and Replay persist equivalent state."},
