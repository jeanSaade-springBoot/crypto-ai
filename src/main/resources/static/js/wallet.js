...
// FIX-031: Crypto Account configuration is resolved strictly from the logged-in user.
// No user id is sent from the browser, so one authenticated session cannot edit another user's account.
async function loadCryptoAccount() {
    try {
        const account = await api('/api/crypto-account');
        byId('crypto-account-user').textContent = account.username || 'Logged-in user';
        byId('crypto-account-exchange').value = account.exchangeCode || 'BINANCE';
        byId('crypto-account-label').value = account.accountLabel || 'Primary account';
        byId('crypto-account-mode').value = account.executionMode || 'PAPER';
        byId('crypto-account-max-order').value = account.maxOrderUsdt ?? 10;
        byId('crypto-account-max-exposure').value = account.maxTotalExposureUsdt ?? 50;
        byId('crypto-account-max-positions').value = account.maxOpenPositions ?? 3;
        byId('crypto-account-max-daily-loss').value = account.maxDailyLossUsdt ?? 10;
        byId('crypto-account-credential-state').textContent = account.credentialsConfigured
            ? `Credentials saved for ${account.apiKeyMasked || 'this Binance account'}. Secret is never returned to the browser.`
        ...
        showMessage(`Crypto Account: ${error.message}`, true);
    }
}

byId('crypto-account-form').addEventListener('submit', async event => {
    event.preventDefault();
    try {
        const account = await api('/api/crypto-account', {
            method: 'PUT',
        ...
        byId('crypto-account-api-secret').value = '';
        byId('crypto-account-clear-credentials').checked = false;
        showMessage('Your Crypto Account configuration was saved. Trading behavior was not changed.');
    } catch (error) {
        showMessage(error.message, true);
    }
});

// Load the user-scoped account independently from the shared wallet dashboard data.
loadCryptoAccount();
