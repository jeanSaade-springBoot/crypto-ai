...
function renderFix129(result) {
    ...
    const summary = document.getElementById('fix129-summary');
    const body = document.getElementById('fix129-body');
    if (!panel || !summary || !body) return;
    panel.classList.remove('hidden');
    body.replaceChildren();
    if (!result || result.status !== 'fulfilled') {
        summary.textContent = 'Ingestion timing history could not be loaded.';
        return;
    }
    const data = result.value;
    ...
    for (const row of data.rows || []) {
        const tr = document.createElement('tr');
        for (const value of [row.symbol, row.interval_code, row.candle_open_time, row.block_start,
            row.stage, row.started_at, row.finished_at, row.elapsed_ms, row.outcome, row.thread_name, row.correlation_id]) {
            const td = document.createElement('td');
            td.textContent = String(value ?? '');
            tr.appendChild(td);
        }
...
document.addEventListener('click', async event => {
    const button = event.target.closest('[data-fix129-proven-id]');
    if (!button) return;
    button.disabled = true;
    try {
        const results = await Promise.allSettled([api(`/api/administration/regression-tests/proven-trades/${encodeURIComponent(button.dataset.fix129ProvenId)}/fix129`)]);
        renderFix129(results[0]);
        document.getElementById('fix129-panel')?.scrollIntoView({behavior:'smooth'});
    } finally { button.disabled = false; }
});
