...
function chartUrl(row, filterState){
  ...
  if(row.rowKind==='DONE'){
    ...
    if(filterState){params.set('signalSymbol',filterState.symbol||'ALL');params.set('signalPeriod',filterState.period||'1d');params.set('signalType',filterState.type||'BLOCKED_BUY');}
...
class Browser{
 ...
 async init(initialSymbol, initialFilters={}){
   await this.loadSymbols();
   // FIX-111: View-chart navigation is a full Dashboard navigation, so explicitly restore
   // the independent Signals browser criteria carried in the deep link before loading rows.
   // Header symbol remains only the fallback when no saved Signal symbol was supplied.
   const requestedSymbol=String(initialFilters.symbol||initialSymbol||'').toUpperCase();
   if(requestedSymbol)this.setSymbol(requestedSymbol,false);
   if(initialFilters.period&&this.$('period'))this.$('period').value=initialFilters.period;
   if(initialFilters.type&&this.$('type'))this.$('type').value=initialFilters.type;
