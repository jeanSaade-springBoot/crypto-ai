...
/** FIX-129: Production incident history for the selected review window. This is
...
public class Fix129DiagnosticsController {
    private final JdbcTemplate jdbc;

    @GetMapping("/runs/{id}/fix129")
    ...
    @GetMapping("/proven-trades/{id}/fix129")
    ...
    private Map<String,Object> rows(Map<String,Object> window) {
        List<Map<String,Object>> rows = jdbc.queryForList("""
                SELECT * FROM fix129_stage_timing
                WHERE symbol=? AND finished_at>=? AND started_at<=?
                ORDER BY started_at,id LIMIT 501
